    ...
    reducers: {
        addToCart: {
            reducer(state: CatalogState, action: PayloadAction<any>) {
                const existingCartItem = state.cartItems.find((item) => item.name === action.payload.name);
                if(existingCartItem) {
                    existingCartItem.quantity++
                    existingCartItem.lineTotal = existingCartItem.lineTotal + existingCartItem.price;
                } else {
